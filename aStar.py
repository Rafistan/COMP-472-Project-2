...
def AStar(game, heuristic_number, puzzle_number):
    # start_time time
    ...
    solutions_path = f".\solutions\AStar-h{heuristic_number}-sol-{puzzle_number}.txt"
    search_path = f".\search\AStar-h{heuristic_number}-search-{puzzle_number}.txt"
    print(f"AStar h{heuristic_number} puzzle #{puzzle_number}")
    # write solution for AStar for the board and the gas, per heuristic, respectively
    ...
    while not game.is_game_over(present_game_board):
        ...
        for possible_move in possible_moves:
            ...
            new_distance_travelled = present_game.distance_travelled

            # simply add a new distance trabelled if the new direction is different from the previous direction
            ...
            if not possible_move == present_game.last_move:
                if heuristic_number == 1:
                    new_cost = h1(new_board) + new_distance_travelled
                    with open(search_path, "a+") as search:
                        search.write(
                            f"\n{h1(new_board) + present_game.distance_travelled}\t{present_game.distance_travelled}\t{h1(new_board)}\t{present_game_board[0][0:6]}{present_game_board[1][0:6]}{present_game_board[2][0:6]}{present_game_board[3][0:6]}{present_game_board[4][0:6]}{present_game_board[5][0:6]}")
                elif heuristic_number == 2:
                    new_cost = h2(new_board) + new_distance_travelled
                    with open(search_path, "a+") as search:
                        search.write(
                            f"\n{h2(new_board) + present_game.distance_travelled}\t{present_game.distance_travelled}\t{h2(new_board)}\t{present_game_board[0][0:6]}{present_game_board[1][0:6]}{present_game_board[2][0:6]}{present_game_board[3][0:6]}{present_game_board[4][0:6]}{present_game_board[5][0:6]}")
                elif heuristic_number == 3:
                    new_cost = h3(new_board) + new_distance_travelled
                    with open(search_path, "a+") as search:
                        search.write(
                            f"\n{(h3(new_board) + present_game.distance_travelled)}\t{present_game.distance_travelled}\t{h3(new_board)}\t{present_game_board[0][0:6]}{present_game_board[1][0:6]}{present_game_board[2][0:6]}{present_game_board[3][0:6]}{present_game_board[4][0:6]}{present_game_board[5][0:6]}")
                elif heuristic_number == 4:
                    new_cost = h4(new_board) + new_distance_travelled
                    with open(search_path, "a+") as search:
                        search.write(
                            f"\n{(h4(new_board) + present_game.distance_travelled)}\t{present_game.distance_travelled}\t{h4(new_board)}\t{present_game_board[0][0:6]}{present_game_board[1][0:6]}{present_game_board[2][0:6]}{present_game_board[3][0:6]}{present_game_board[4][0:6]}{present_game_board[5][0:6]}")
            ...
            new_game.distance_travelled = new_cost
        ...
        if len(list_of_open_games) == 0 and (len(car_object.possible_moves(list_of_closed_games[len(list_of_closed_games) - 1].board, list_of_closed_games[
            ...
            print("No Solution")
            with open(solutions_path, "w+") as solution:
    ...
    with open(f".\\analysis.txt", "a") as analysis_file:
        analysis_file.write(f"\n{puzzle_number}\t A*\t{heuristic_number}\t{solution_path_length}\t{search_length} \t{end_time - start_time} ")
