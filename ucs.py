...
def UCS(game, puzzle_number):
    ...
    solutions_path = f".\solutions\\ucs-sol-{puzzle_number}.txt"
    search_path = f".\search\\ucs-search-{puzzle_number}.txt"
    print(f"UCS puzzle #{puzzle_number}")
    # write solution for UCS for the board and the gas, per heuristic, respectively
    ...
    while not game.is_game_over(present_game_board):
        ...
        for possible_move in possible_moves:
            ...
            if not possible_move == present_game.last_move:
                new_cost = present_game.cost + 1
            # append f(n), g(n) and h(n) and the board to the file
            with open(search_path, "a+") as search:
                search.write(
                    f"\n{new_cost}\t{new_cost}\t0\t{present_game_board[0][0:6]}{present_game_board[1][0:6]}{present_game_board[2][0:6]}{present_game_board[3][0:6]}{present_game_board[4][0:6]}{present_game_board[5][0:6]}")
            ...
            # Create a new state based on the values set above
    ...
    with open(f".\\analysis.txt", "a") as analysis_file:
        analysis_file.write(f"\n{puzzle_number}\t UCS\tNA\t{solution_path_length}\t{search_length} \t{end_time - start_time} ")
